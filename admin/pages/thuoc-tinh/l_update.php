...
<div class="main-update">
    ...
    <form class="form-group" action="pages/thuoc-tinh/action.php?req=i_update" method="post">
    ...
    <div class="col">
        <?php if (count($thuoctinh__Get_By_Id_Loai) > 0) : ?>
            <div class="form-check">
                <div class="row">
                    <?php foreach ($thuoctinh__Get_By_Id_Loai as $index => $item) : ?>
                        <input type="hidden" id="idtt[]" value="<?= $item->idtt ?>" name="idtt[]" required>
                        <!-- Đưa chỉ số index vào input hidden -->
                        ...
                        <div class="col-4">
                            ...
                            <select class="form-select" aria-label=".trangthai" id="trangthai" name="trangthai">
                                ...
                                <option value="0" <?= $item->trangthai == 0 ? "selected" : "" ?>>Tạm ẩn</option>
                            </select>
                        </div>
                        <div class="col mt-2">
                            <br>
                            <div class="form-check form-check-inline">
                                <!-- Đặt name là "is_num[<?= $index ?>]" -->
                                <input class="form-check-input checkbox" type="radio" id="is_num_<?= $index ?>" value="0" name="is_num[<?= $index ?>]" required <?= $item->is_num == 0 ? 'checked' : '' ?>>
                                <label class="form-check-label" for="is_num_<?= $index ?>">Kiểu chữ</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <!-- Đặt name là "is_num[<?= $index ?>]" -->
                                <input class="form-check-input checkbox" type="radio" id="is_num_<?= $index ?>" value="1" name="is_num[<?= $index ?>]" required <?= $item->is_num == 1 ? 'checked' : '' ?>>
                                <label class="form-check-label" for="is_num_<?= $index ?>">Kiểu số</label>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
