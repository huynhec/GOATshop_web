    ...
    <form class="form-group" action="pages/san-pham/action.php?req=update" method="post">
        ...
        <div class="col">
            ...
            <?php foreach ($loaiSp__Get_All as $item) : ?>
                <div class="form-check form-check-inline">
                    <input class="btn-check" type="radio" id="maloai<?= $item->maloai ?>" value="<?= $item->maloai ?>" name="maloai" <?= $item->maloai == $sanPham__Get_By_Id->maloai ? 'checked' : '' ?> <?= $item->maloai == $sanPham__Get_By_Id->maloai ? "onclick='d_update_obj()'" : "onclick='d_add_obj(`$item->maloai`)'" ?>>
